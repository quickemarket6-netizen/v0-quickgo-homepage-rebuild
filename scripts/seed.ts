 ...
 * Il utilise upsert/delete+insert pour repartir d'un état propre.
 */
...
function err(label: string, e: unknown) { console.error("  ✗", label, e) }
...
} catch (e) { err("users", e); process.exit(1) }
...
if (profErr) err("profiles", profErr.message)
else ok(`${profiles.length} profils`)

// ─── 3. Catégories ────────────────────────────────────────────────────────────
console.log("\n── 3. Catégories ─────────────────────────────────────────────")

// Supprime d'abord les catégories de test pour éviter les doublons
...
if (catErr) { err("categories", catErr.message); process.exit(1) }
...
if (vendErr) { err("vendors", vendErr.message); process.exit(1) }
ok(`${vendors?.length} vendeurs`)

const v1 = vendors![0].id  // TechShop
const v2 = vendors![1].id  // Chez Fatima

// ─── 5. Portefeuilles vendeurs ────────────────────────────────────────────────
console.log("\n── 5. Wallets vendeurs ───────────────────────────────────────")
...
if (walletErr) err("vendor_wallets", walletErr.message)
else ok("2 wallets")
...
const { data: products, error: prodErr } = await sb.from("products").insert([
  ...
  { vendor_id: v2, category_id: catMap["restaurant"],   name: "Beignets haricots ×10",   description: "Beignets de haricots dorés, accompagnement de piment.",              price: 1200, original_price: null,    rating: 4.7, is_available: true, is_featured: false, stock_quantity: 100},
...
if (prodErr) { err("products", prodErr.message); process.exit(1) }
...
if (drvErr) err("drivers", drvErr.message)
...
type OrderInsert = {
  order_number: string; customer_id: string; vendor_id: string; driver_id: string | null
  status: string; subtotal: number; delivery_fee: number; discount: number; total: number
  total_amount: number; delivery_address: string; created_at: string; payment_method: string; payment_status: string
}

const ordersToInsert: OrderInsert[] = [
  // Commandes client1 chez TechShop
  ...
  // Commandes client2 chez TechShop
...
if (ordErr) { err("orders", ordErr.message); process.exit(1) }
ok(`${orders?.length} commandes`)

// ─── 9. Articles de commande ──────────────────────────────────────────────────
console.log("\n── 9. Articles de commande ───────────────────────────────────")

type ItemInsert = { order_id: string; product_id: string; product_name: string; quantity: number; unit_price: number; total_price: number }
const items: ItemInsert[] = []

function addItem(order: { id: string }, productName: string, qty: number) {
  const p = prodMap[productName]
  if (!p) return
  items.push({ order_id: order.id, product_id: p.id, product_name: p.name, quantity: qty, unit_price: p.price, total_price: p.price * qty })
}
...
if (orders) {
  ...
  addItem(orders[7], "Beignets haricots ×10", 2)
...
if (itemErr) err("order_items", itemErr.message)
...
const { error: promoErr } = await sb.from("promo_codes").insert([
  ...
  { code: "FATIMA20",   title: "Spécial Chez Fatima",     description: "-2000 F sur votre commande chez Fatima",      discount_type: "fixed",      discount_value: 2000, min_order_amount: 5000, max_uses: 100, current_uses: 33,  valid_until: null,                                               is_active: true },
])

if (promoErr) err("promo_codes", promoErr.message)
...
const commLogs = [
  // TechShop — derniers 7 jours
  ...[6, 5, 4, 3, 2, 1, 0].map((d) => ({
    vendor_id: v1,
    gross_amount: randBetween(80000, 250000),
    commission_rate: 5,
    get quickgo_commission() { return Math.round(this.gross_amount * 0.05) },
    get vendor_net_amount() { return this.gross_amount - this.quickgo_commission },
    created_at: new Date(new Date().setHours(23, 59, 0, 0) - d * 86400000).toISOString(),
  })),
  // Chez Fatima — derniers 7 jours
  ...[6, 5, 4, 3, 2, 1, 0].map((d) => ({
    vendor_id: v2,
    gross_amount: randBetween(12000, 45000),
    commission_rate: 5,
    get quickgo_commission() { return Math.round(this.gross_amount * 0.05) },
    get vendor_net_amount() { return this.gross_amount - this.quickgo_commission },
    created_at: new Date(new Date().setHours(23, 59, 0, 0) - d * 86400000).toISOString(),
...
// Résoudre les getters
const commLogsResolved = commLogs.map((l) => ({
  vendor_id: l.vendor_id,
  gross_amount: l.gross_amount,
  commission_rate: l.commission_rate,
  quickgo_commission: Math.round(l.gross_amount * 0.05),
  vendor_net_amount: l.gross_amount - Math.round(l.gross_amount * 0.05),
  created_at: l.created_at,
}))

const { error: commErr } = await sb.from("commission_logs").insert(commLogsResolved)
if (commErr) err("commission_logs", commErr.message)
else ok(`${commLogsResolved.length} logs de commission`)
...
if (notifErr) err("notifications", notifErr.message)
...
║  🍽️  Vendeur   fatima@quickgo.cm  (Chez Fatima Resto) ║
