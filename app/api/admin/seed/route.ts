 ...
 *
 * Crée 6 utilisateurs + toutes les données de démo.
 * Protégée par un secret en query param.
 */
...
export async function GET(req: NextRequest) {
  ...
  if (!url || !key) {
    return NextResponse.json({ error: "Variables d'environnement manquantes (SUPABASE_SERVICE_ROLE_KEY)" }, { status: 500 })
  }
  ...
  function err(msg: string, e: unknown) { errors.push("✗ " + msg + ": " + String(e)) }
  ...
  try {
    ...
    const { data: cats } = await sb.from("categories").insert([
    ...
    if (!cats) throw new Error("Échec catégories")
    ...
    const { data: dr } = await sb.from("drivers").insert({ user_id: driverId, status: "online", vehicle_type: "motorcycle", rating: 4.6, total_deliveries: 342, total_earnings: 512000 }).select().single()
    ok("1 livreur")
    ...
    const { data: orders } = await sb.from("orders").insert([
    ...
    if (!orders) throw new Error("Échec commandes")
    ...
    await sb.from("order_items").insert(itemsRaw)
    ok(`${itemsRaw.length} articles de commande`)
    ...
    await sb.from("promo_codes").insert([
      ...
      { code: "TECH15",     title: "Tech Week",           description: "-15% électronique",                  discount_type: "percentage", discount_value: 15,   min_order_amount: 50000, is_active: true, valid_until: new Date(Date.now() + 5  * 86400000).toISOString() },
    ...
    await sb.from("commission_logs").insert(commLogs)
    ok(`${commLogs.length} logs de commission`)
